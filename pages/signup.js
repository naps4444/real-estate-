...
import { account } from '@/services/appwrite'; // Appwrite service
// import { signIn } from 'next-auth/react'; // Next-auth for Google login
...
export default function Signup() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Appwrite signup
      await account.create('unique()', email, password, `${name} ${lastName}`);
    ...
    <div className='flex items-center'>
      <div className='lg:w-6/12 container'>
        ...
        <div className='w-10/12 px-[12px] mx-auto py-12'>
          ...
          {/* Google OAuth Button */}
          <button onClick={handleGoogleSignIn} className='w-full flex items-center justify-center gap-2 py-3 rounded-lg mt-4 border-[1px] border-black'>
          ...
          <div className='flex gap-2 justify-center mt-8 items-center'>
            ...
            <a href='/login' className='text-[#3D9970]'>Sign in</a>
          </div>
        </div>
      </div>
